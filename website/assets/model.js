...
export function scope(data,year,volume="all",entity="all",anchorYear=year,responsibility="all",priority="all"){const attribution=new Map((data.snapshots[anchorYear]||[]).map(s=>[s.id,s]));return (data.snapshots[year]||[]).filter(s=>{const a=attribution.get(s.id);return(volume==="all"||data.byId.get(s.id).volume===volume)&&(entity==="all"||(a&&entityKeys(a).includes(entity)))&&(responsibility==="all"||a?.responsibility===responsibility)&&(priority==="all"||(data.byId.get(s.id).priority||"unavailable")===priority)});}
export function movement(data,year,volume='all',entity='all',anchorYear=year,responsibility="all",priority="all"){const now=scope(data,year,volume,entity,anchorYear,responsibility,priority),before=scope(data,year-1,volume,entity,anchorYear,responsibility,priority),prior=new Map(before.map(s=>[s.id,s]));const assessmentRate=rate(now);const m={...assessmentRate,implementedRate:assessmentRate.implemented,year,opening:before.filter(s=>!terminal(s.status)).length,issued:0,reopened:0,implemented:0,other:0,closing:now.filter(s=>!terminal(s.status)).length};for(const s of now){const p=prior.get(s.id);if(!p)m.issued++;if(p&&terminal(p.status)&&!terminal(s.status))m.reopened++;if((!p||!terminal(p.status))&&terminal(s.status))m[s.status==='implemented'?'implemented':'other']++;}const implementedClosedRate=now.filter(s=>terminal(s.assessment)).length;return {...m,implementedClosed:m.implemented+m.other,implementedClosedRate,implementedClosedValue:m.denominator?implementedClosedRate/m.denominator*100:null};}
